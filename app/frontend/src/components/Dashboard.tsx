        ...
        <div className="card p-10 shadow-lg border-none flex flex-col group overflow-hidden">
          ...
          <div className="space-y-6 flex-1 flex flex-col justify-center">
            <div className="flex items-end justify-between">
              ...
              <span className="text-xs font-black text-[var(--on-surface-variant)] uppercase tracking-widest">${(state.goal?.desiredDownPayment || state.goal?.targetAmount || 0).toLocaleString()} downpayment target</span>
            </div>
            ...
            <div className="space-y-3">
              <div className="h-3 w-full bg-[var(--surface-container-high)] rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(100, ((state.goal?.currentSavings || 0) / (state.goal?.desiredDownPayment || state.goal?.targetAmount || 1)) * 100)}%` }}
                ></div>
              </div>
              <p className="text-[10px] text-[var(--on-surface-variant)] font-black uppercase tracking-[0.1em] text-right">
                {Math.round(((state.goal?.currentSavings || 0) / (state.goal?.desiredDownPayment || state.goal?.targetAmount || 1)) * 100)}% funded
              </p>
            </div>
          </div>

          <div className="pt-10 mt-10 border-t border-[var(--outline-variant)]">
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-[var(--on-surface-variant)] mb-2">{state.payFrequency} Contribution Target</p>
